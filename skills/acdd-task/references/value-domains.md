...
The validator scans each declared root, prunes dependency, cache, generated-build, and VCS directories, and applies exact terms. It is bounded to 20,000 inspected text files and 256 matching files per contract. Exceeding a bound fails with an instruction to narrow roots or use more precise identifiers; it does not demand an unbounded disposition list. Every file found by those terms must be classified with a pipeline role or `unrelated` plus rationale. Explicit `files` entries are path classifications and remain valid when their current content does not match a discovery term. All non-unrelated files must be task inputs. Required roles are producer, writer, schema, reader, public-type, and proof.

Choose terms that identify additional contract candidates rather than generic words such as `value`, `status`, or `visibility`. Discovery closure proves every exact-search result was reconciled; architecture review and dependency traversal still prove repository-wide relationship closure.
