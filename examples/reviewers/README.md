...
- [`task-adapter.yaml`](.acdd-legacy/task-adapter.yaml) owns isolated inbound
  `architecture/v1`: four read-only inspectors, then one tool-free coordinator.
- [`implementation-adapter.yaml`](.acdd-legacy/implementation-adapter.yaml) owns the
  terminal task `review/v1`.
- [`plan-adapter.yaml`](.acdd-legacy/plan-adapter.yaml) owns bounded planning
