...
## Task delivery: 5 gates, 9 checks
...
- **`design-basis`** — explains what will change, why it matters, which
  systems and callers are affected, and which effects must never occur.
...
- **`decomposition`** — shows that every required change belongs to one
  bounded subtask and that dependent work has a clear order.
- **`matrix`** — maps the affected producers, consumers, data flow,
  authorities, backends, reads, and writes so no relevant path is implicit.
...
A subtask is one bounded change slice, not a to-do item. Its `writes` and
`reads` name either a declared Input or a path inside a declared directory
Input. That lets several subtasks share `src` without repeating it, while each
still names the files it changes. Its `acceptance` describes an observable
result, and `dependsOn` makes execution order explicit. IDs are unique,
dependencies are acyclic, and conflicting reads/writes must be serialized.
...
- **Addition** narrows or extends the work with its own paths and
  `dependsOn: [original]`. Finish the original subtask's TDD Red → Green before
  starting the addition's Red test.
- **Replacement** uses `supersedes: original`. The original contract remains in
  the record, but its state is now derived as superseded by that one successor.
  A subtask has at most one direct replacement.
...
receipt. Source contracts are verification records, not subtask gates or subtask receipts. A
Plan-only addition or replacement does not stale a global Build, Review, or
Handoff receipt; changing that receipt's actual inputs still does.
...
- **`runtime-and-integration`** — runs the repository's integrated command
  after the focused tests are green. It demonstrates the intended runtime
  behavior and the relevant integration or quality guarantees on the settled
  tree.

### 4. Review

Challenge the settled change across the task profile's **parity**, **security**,
and **code** dimensions. The planning profile instead checks its settled set for
...
- **`repository-handoff`** — confirms required closure actions, changed
  derived artifacts, and the absence of unresolved blockers.
- **`process-report`** — report-kind check. ACDD synthesizes
  `acdd/process-report/1` JSON from the document's receipts and evidence
  (gates, fingerprints, subtask ids, review refs). The Task adapter must bind
  the check; it is not a second owner shell workflow.

## Planning profile

`acdd/plan/v1` creates or improves a bounded planning set; it never implements
source changes. **Design** uses `design-basis` and `plan-shape`; it may be
`inapplicable` only as `plan.no-artifact`. **Decompose** uses `decomposition`
and `matrix` to make the planning set coherent and bounded. **Review** checks
that set independently for `completeness` and `consistency`. Use the Plan and
Review adapters described below. See the [plan example](examples/plan-example.md).
...
2. **Evidence is real** — artifacts are local, checksummed, unique, single-record, and pass their declared outcome; basis evidence covers its scope; review terminals carry `verdict: pass`; process reports match `acdd/process-report/1`. Every Contract source part must have one matching binding in the same bundle.
3. **Receipt binds evidence** — receipt, bundle, and check fingerprints agree.
4. **Receipt binds state** — a receipt binds the gate’s profile definition, adapter
   bindings, prompt fragment, and relevant inputs. It does not bind the Plan:
   adding or replacing a subtask alone leaves global receipts current. A stale
   receipt is renewed only by current evidence for that same gate; any other
   stale or invalid state still blocks finalization.
...
11. **Review complete** — review transcript preserves every raw reviewer
    response. Its confirmation reviewer, also a raw session, issues the final
    pass terminal; it acknowledges exactly all raw sessions through
    `reviewedSessionUuids`, declares a non-empty scope, and covers the profile's
    `reviewDimensions`.
...
`.acdd/prompts/<gate>-<check>.md`), must stay below its directory, and its
SHA-256 enters the gate fingerprint. For command and basis checks, `cwd` stays
below the workspace root and `timeoutSeconds` (default 300) bounds the command.
A review host instead combines the base review skill and fragment, then expands
its launch template. The core does not compose prompts, choose models, or replace
ownership, check requirements, evidence kind, terminal policy, or an invariant.

Evidence artifacts land in `<adapter dir>/artifacts/` (`.acdd/artifacts/` in a
consuming repository). Successful check artifacts are immutable, gitignored
...
`record` may leave an orphan JSONL;
retrying the same `--id` overwrites it when that id is not yet in the document.

## Commands

Adapters load from the discovery scope below (not a recursive walk of every
`.acdd/` under the workspace). Every `*.yaml` with `apiVersion: acdd/adapter/v1`
directly inside a selected `.acdd/` root is considered. An adapter with no gate
in the active profile is ignored; one that declares an active gate is indexed by
role and checked strictly, including unknown bindings. Symlinked adapter files
...
uv run acdd validate task.md profiles/task/v1.yaml

uv run acdd fingerprint task.md profiles/task/v1.yaml --gate build/v1

uv run acdd record task.md profiles/task/v1.yaml \
...
uv run acdd finalize task.md profiles/task/v1.yaml --gate build/v1 --id build.bundle

uv run acdd contract-subtask task.md profiles/task/v1.yaml --workspace-root . \
  --subtask newly-discovered-slice --id contract.newly-discovered-slice

uv run acdd review task.md profiles/task/v1.yaml \
...
uv run acdd finalize task.md profiles/task/v1.yaml --gate build/v1 \
...
does not launch reviewers, choose models, or authenticate identity; the consuming
workflow owns those guarantees. See [DESIGN.md](DESIGN.md) for the host contract
and Pi integration boundary.

## Install adapters

Adapters are `apiVersion: acdd/adapter/v1` YAML files under `.acdd/`.

**Discovery scope** (from the document path under `--workspace-root`):

1. **Single-repo (default for most consumers)** — when no platform `.acdd`
   roots exist, load the workspace-root `.acdd/`.
2. **Optional multi-root platform** — if `planner/.acdd` or
   `contextunity/.acdd` exist, only those roots load (not a recursive scan).
3. **Project opt-in** — document under `projects/<name>/` with a local
   `.acdd/` → only that project (no platform fallback).
4. **Unused project** — document under `projects/<name>/` without `.acdd/` →
   ACDD is not used for that project.

Task delivery needs roles `task`, `implementation`, and `review`. Planning needs
`plan` and `review`. A Review binding is a host launch template; register the
finished transcript with `acdd review`. Handoff requires `repository-handoff`
plus core-synthesized `process-report`. See
[DESIGN.md](DESIGN.md#adapters-and-prompt-context).
