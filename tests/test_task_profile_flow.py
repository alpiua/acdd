...
""", encoding="utf-8")
    ...
    def run(command: str, *args: str) -> None:
        assert main([command, str(document), str(profile), "--workspace-root", str(tmp_path), *args]) == 0

    for gate, check in (("design/v1", "design-basis"), ("design/v1", "plan-shape"),
                        ("contract/v1", "decomposition"), ("contract/v1", "matrix")):
        args = ("--gate", gate, "--check", check, "--id", f"{gate[:3]}.{check}")
        run("record", *args, *(("--classified-ref", "design.md=task") if check in {"design-basis", "decomposition", "matrix"} else ()))
        if check == "plan-shape":
            run("finalize", "--gate", gate, "--id", "design.bundle")
        if check == "matrix":
            run("record", "--gate", "contract/v1", "--check", "executable-proof", "--id", "contract.proof")
            run("finalize", "--gate", "contract/v1", "--id", "contract.bundle")
    run("record", "--gate", "build/v1", "--check", "runtime-and-integration", "--id", "build.runtime")
    run("finalize", "--gate", "build/v1", "--id", "build.bundle")
    transcript = adapters / "artifacts" / "review.jsonl"
    transcript.parent.mkdir(exist_ok=True)
    transcript.write_text("\n".join(json.dumps(row) for row in (
        {"type": "review_raw", "reviewerSessionUuid": "00000000-0000-4000-8000-000000000002",
         "raw": '{"findings": []}'},
        {"type": "review_terminal", "evidenceId": "review.independent", "gate": "review/v1",
         "check": "independent-review", "scope": ["src/app.py"], "performedChecks": ["parity", "security", "code"],
    ...
    run("review", "--gate", "review/v1", "--check", "independent-review", "--id", "review.independent",
        "--transcript", ".acdd/artifacts/review.jsonl", "--author-uuid", "00000000-0000-4000-8000-000000000001",
