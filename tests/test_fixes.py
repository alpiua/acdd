...
""", encoding="utf-8")
    ...
    artifact.write_text(json.dumps(
        {"type": "command_run", "evidenceId": "e1", "gate": "build/v1", "check": "c", "exitCode": 1}) + "\n",
        encoding="utf-8")
    gate = Gate("build/v1", "implementation", (Check("c", "basis", "success"),), ("source",), ("pass",))
    ...
    child = {"kind": "basis", "id": "e1", "gate": "build/v1", "check": "c", "issuerRole": "implementation",
             "artifactSha256": sha256(artifact), "inputFingerprint": fingerprint,
             "basisRef": "basis.jsonl", "scope": [], "classifiedRefs": []}
    bundle = {"kind": "bundle", "id": "b", "gate": "build/v1", "issuerRole": "implementation",
              "checkEvidence": ["e1"], "inputFingerprint": fingerprint}
    document.evidence = [child, bundle]
    document.receipts = [{"gate": "build/v1", "status": "pass", "evidence": "bundle=b",
                          "fingerprint": fingerprint, "recordedAt": "now"}]
...
def test_basis_scope_must_cover_declared_inputs(tmp_path: Path):
    (tmp_path / "artifact.jsonl").write_text(json.dumps(
        {"type": "command_run", "evidenceId": "e1", "gate": "build/v1", "check": "c", "exitCode": 0}) + "\n",
        encoding="utf-8")
    artifact = tmp_path / "artifact.jsonl"
    gate = Gate("build/v1", "implementation", (Check("c", "basis", "success"),), ("source",), ("pass",))
    document = Document(title="T", inputs=[{"type": "source", "path": "src/app.py"}],
                        evidence=[], receipts=[], subtasks=[], path=tmp_path / "task.md")
    evidence = {"kind": "basis", "id": "e1", "gate": "build/v1", "check": "c",
                "issuerRole": "implementation", "scope": [], "classifiedRefs": [],
                "artifactSha256": sha256(artifact), "basisRef": "artifact.jsonl"}
...
def test_multi_record_command_artifact_is_rejected(tmp_path: Path):
    ...
    record = {"type": "command_run", "evidenceId": "e1", "gate": "build/v1", "check": "c", "exitCode": 0}
    artifact.write_text(json.dumps(record) + "\n" + json.dumps(record) + "\n", encoding="utf-8")
    document.evidence = [{"kind": "command", "id": "e1", "gate": "build/v1", "check": "c",
                          "issuerRole": "implementation", "artifactSha256": sha256(artifact),
...
def test_validator_rejects_unhashable_bundle_member(tmp_path: Path):
    gate = Gate("build/v1", "implementation", (Check("c", "command", "success"),), (), ("pass",))
    document = Document(title="T", inputs=[], evidence=[], receipts=[], subtasks=[], path=tmp_path / "t.md")
    fingerprint = fingerprint_for_gate(document, gate, tmp_path, None)
    document.evidence = [{"kind": "bundle", "id": "b", "gate": "build/v1",
                          "issuerRole": "implementation", "checkEvidence": [{}],
...
def test_missing_executable_returns_error_not_traceback(core):
    ...
    adapter.write_text(adapter.read_text(encoding="utf-8").replace(
        "argv: [python3, -c, \"print('green')\"]", "argv: [nonexistent-binary-xyz]"), encoding="utf-8")
...
def test_transitive_dependency_serializes_conflict(tmp_path: Path):
    gate = Gate("build/v1", "implementation", (Check("c", "command", "success"),), (), ("pass",))
    document = Document(title="T", inputs=[{"type": "source", "path": "x.py"}], evidence=[],
                        receipts=[{"gate": "build/v1", "status": "pending", "evidence": "pending",
                                   "fingerprint": "pending", "recordedAt": "pending"}],
                        subtasks=[Subtask("a", ("x.py",), (), "A"), Subtask("b", (), (), "B", ("a",)),
                                  Subtask("c", ("x.py",), (), "C", ("b",))],
                        path=tmp_path / "task.md")
    errors = validate(document, Profile("t", [gate]), workspace_root=tmp_path)
...
def test_adapter_cwd_escape_is_rejected(core):
    doc, profile, adapter = core
    adapter.write_text(adapter.read_text(encoding="utf-8").replace("cwd: .", "cwd: /etc"), encoding="utf-8")
    assert main(["record", *_args(doc, profile, adapter), "--gate", "build/v1",
...
""", encoding="utf-8")
    with pytest.raises(AdapterError, match="timeoutSeconds"):
        load_adapter(path)
    path.write_text(path.read_text(encoding="utf-8").replace("timeoutSeconds: 0", "timeoutSeconds: 5"),
                    encoding="utf-8")
...
def test_partial_receipt_must_be_fully_pending(tmp_path: Path):
    gate = Gate("build/v1", "implementation", (Check("c", "command", "success"),), (), ("pass",))
    document = Document(title="T", inputs=[], evidence=[],
                        receipts=[{"gate": "build/v1", "status": "partial", "evidence": "garbage",
                                   "fingerprint": "garbage", "recordedAt": "never"}],
                        subtasks=[], path=tmp_path / "task.md")
    errors = validate(document, Profile("t", [gate]), workspace_root=tmp_path)
...
def test_document_profile_mismatch_is_rejected(core):
    ...
    doc.write_text(doc.read_text(encoding="utf-8").replace(
        "planning_profile: test/v1", "planning_profile: other/v1"), encoding="utf-8")
...
def test_unrelated_input_type_does_not_stale_receipt(core):
  ...
  - {type: source, path: src/app.py}""",
        """paths:
  - {type: source, path: src/app.py}
  - {type: docs, path: docs/guide.md}"""), encoding="utf-8")
    errors = validate(load_document(doc), load_profile(profile),
                      adapters=[load_adapter(adapter)], workspace_root=doc.parent)
    assert not any(error.invariant == 4 for error in errors)
...
def test_note_column_rules(tmp_path: Path):
    ...
    def errors_for(status: str, note: str | None):
        cells = ["build/v1", status, "pending", "pending", "pending"] + ([note] if note is not None else [])
        document = Document(title="T", inputs=[], evidence=[],
                            receipts=[dict(zip(("gate", "status", "evidence", "fingerprint", "recordedAt", "note"), cells))],
                            subtasks=[], path=tmp_path / "t.md")
...
def test_acdd_discovery_loads_adapters(core):
    ...
    assert main(["record", *args, "--gate", "build/v1",
                 "--check", "runtime-and-integration", "--id", "build.check"]) == 0
...
def test_adapter_outside_workspace_is_rejected(core):
    ...
    assert main(["validate", str(doc), str(profile), "--workspace-root", str(doc.parent),
                 "--adapter", "implementation=/etc/passwd"]) == 2
...
def test_overflow_receipt_row_is_a_shape_error(core):
    ...
    doc.write_text(doc.read_text(encoding="utf-8").replace(
        ...
        "| build/v1 | pending | pending | pending | pending | note | junk |"), encoding="utf-8")
    errors = validate(load_document(doc), load_profile(profile),
                      adapters=[load_adapter(adapter)], workspace_root=doc.parent)
    assert any(error.invariant == 1 for error in errors)
...
def test_review_transcript_non_string_elements_rejected(tmp_path: Path):
    ...
    document = Document(title="T", inputs=[], evidence=[{
        ...
        "reviewerSessionUuid": "00000000-0000-4000-8000-000000000002", "verdict": "pass"}],
...
def test_failed_record_allows_retry_same_id(core):
    ...
    adapter.write_text(adapter.read_text(encoding="utf-8").replace(
        ...
        "argv: [python3, -c, \"import sys; sys.exit(1)\"]"), encoding="utf-8")
    assert main(["record", *_args(doc, profile, adapter), "--gate", "build/v1",
                 "--check", "runtime-and-integration", "--id", "build.retry"]) == 1
    assert (doc.parent / "artifacts" / "build.retry.jsonl").is_file()
    adapter.write_text(adapter.read_text(encoding="utf-8").replace(
        "argv: [python3, -c, \"import sys; sys.exit(1)\"]",
        "argv: [python3, -c, \"print('green')\"]"), encoding="utf-8")
    assert main(["record", *_args(doc, profile, adapter), "--gate", "build/v1",
                 "--check", "runtime-and-integration", "--id", "build.retry"]) == 0


def test_expected_failure_rejects_reserved_exit_codes(tmp_path: Path):
    artifact = tmp_path / "run.jsonl"
    artifact.write_text(json.dumps(
        {"type": "command_run", "evidenceId": "e1", "gate": "build/v1", "check": "c",
         "exitCode": 124}) + "\n", encoding="utf-8")
    gate = Gate("build/v1", "implementation", (Check("c", "command", "expected-failure"),), (), ("pass",))
    document = Document(title="T", inputs=[], evidence=[{
        "kind": "command", "id": "e1", "gate": "build/v1", "check": "c",
        "issuerRole": "implementation", "artifactSha256": sha256(artifact),
        "commandReceipt": "run.jsonl"}], receipts=[], subtasks=[], path=tmp_path / "t.md")
    errors = validate(document, Profile("t", [gate]), workspace_root=tmp_path)
    assert any(error.invariant == 9 for error in errors)
...
""", encoding="utf-8")
    ...
    transcript.write_text(json.dumps({
        "type": "command_run", "evidenceId": "r1", "gate": "review/v1", "check": "independent-review",
        "scope": ["src/"], "performedChecks": ["parity"], "exitCode": 0,
        "verdict": "pass", "authorSessionUuid": "00000000-0000-4000-8000-000000000001",
        "reviewerSessionUuid": "00000000-0000-4000-8000-000000000002",
    }) + "\n", encoding="utf-8")
    document = Document(title="T", inputs=[], evidence=[], receipts=[], subtasks=[],
...
def test_terminal_gate_requires_prior_gates(tmp_path: Path):
    ...
    document = Document(title="T", inputs=[], evidence=[],
                        receipts=[
                            {"gate": "design/v1", "status": "pending", "evidence": "pending",
                             "fingerprint": "pending", "recordedAt": "pending"},
                            {"gate": "build/v1", "status": "pass", "evidence": "bundle=b",
                             "fingerprint": "fp", "recordedAt": "now"},
                        ], subtasks=[], path=tmp_path / "t.md")
...
def test_duplicate_check_evidence_rejected_on_record(core):
    doc, profile, adapter = core
    assert main(["record", *_args(doc, profile, adapter), "--gate", "build/v1",
                 "--check", "runtime-and-integration", "--id", "build.one"]) == 0
    assert main(["record", *_args(doc, profile, adapter), "--gate", "build/v1",
                 "--check", "runtime-and-integration", "--id", "build.two"]) == 2
...
def test_inapplicable_with_empty_bundle_passes(core):
    doc, profile, adapter = core
    assert main(["finalize", *_args(doc, profile, adapter), "--gate", "build/v1",
                 "--id", "build.inapp", "--status", "inapplicable",
