...
def _final(core):
    ...
    assert main(["record", *_args(doc, profile, adapter), "--gate", "build/v1", "--check", "runtime-and-integration", "--id", "build.check"]) == 0
    assert main(["finalize", *_args(doc, profile, adapter), "--gate", "build/v1", "--id", "build.bundle"]) == 0
    return doc, profile, adapter


def _errors(doc: Path, profile: Path, adapter: Path):
    return validate(load_document(doc), load_profile(profile), adapters=[load_adapter(adapter)], workspace_root=doc.parent)


def test_invariant_1_rejects_unknown_status(core):
    doc, profile, adapter = core
    doc.write_text(doc.read_text(encoding="utf-8").replace("| build/v1 | pending", "| build/v1 | impossible"), encoding="utf-8")
...
def test_invariant_3_binds_receipt_and_bundle(core):
    doc, profile, adapter = _final(core)
    doc.write_text(doc.read_text(encoding="utf-8").replace("| build/v1 | pass |", "| build/v1 | pass |", 1).replace(" | sha256:", " | sha256:bad", 1), encoding="utf-8")
...
def test_invariant_5_rejects_wrong_bundle_owner(core):
    doc, profile, adapter = _final(core)
    doc.write_text(doc.read_text(encoding="utf-8").replace("issuerRole: implementation", "issuerRole: task", 1), encoding="utf-8")
    assert any(error.invariant == 5 for error in _errors(doc, profile, adapter))


def test_invariant_6_rejects_unbounded_subtask(core):
    doc, profile, adapter = core
    doc.write_text(doc.read_text(encoding="utf-8").replace("subtasks: []", """subtasks:
  - id: bad
    writes: [outside.py]
    reads: []
...
def _terminal_document(tmp_path: Path, *, kind: str, status: str, reason: str | None = None) -> tuple[Document, Profile]:
    gate = Gate("review/v1" if kind == "review" else "build/v1", "review" if kind == "review" else "implementation",
                (Check("independent-review" if kind == "review" else "runtime-and-integration", kind, "success"),), (),
                ("pass", "inapplicable") if kind != "review" else ("pass",), ("build.no-runnable-source",))
    path = tmp_path / "artifact.jsonl"
    child_id = "child"
    record = {"type": "review_terminal" if kind == "review" else "command_run", "evidenceId": child_id,
              "gate": gate.id, "check": gate.checks[0].id}
    if kind == "review":
    ...
    child = {"kind": kind, "id": child_id, "gate": gate.id, "check": gate.checks[0].id,
             "issuerRole": gate.owner, "artifactSha256": sha256(path), "inputFingerprint": fingerprint,
             {"review": "transcriptRef", "command": "commandReceipt"}[kind]: "artifact.jsonl"}
    if kind == "review":
        child.update({"verdict": "pass", "authorSessionUuid": "00000000-0000-4000-8000-000000000001",
                      "reviewerSessionUuid": "00000000-0000-4000-8000-000000000001"})
    check_evidence = [] if status == "inapplicable" else [child_id]
    bundle = {"kind": "bundle", "id": "bundle", "gate": gate.id, "issuerRole": gate.owner,
              "checkEvidence": check_evidence, "inputFingerprint": fingerprint}
    if reason is not None:
        bundle["reasonCode"] = reason
    document.evidence = ([child] if check_evidence else []) + [bundle]
    document.receipts = [{"gate": gate.id, "status": status, "evidence": "bundle=bundle",
...
def test_invariant_7_requires_distinct_review_uuids(tmp_path: Path):
    ...
    assert any(error.invariant == 7 for error in validate(document, profile, workspace_root=tmp_path))


def test_invariant_8_requires_declared_reason(tmp_path: Path):
    document, profile = _terminal_document(tmp_path, kind="command", status="inapplicable", reason="wrong.reason")
    assert any(error.invariant == 8 for error in validate(document, profile, workspace_root=tmp_path))
