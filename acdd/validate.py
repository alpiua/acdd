...
def _source_contracts(workspace: Path, bundle: dict | None, err: Callable[[int, str], None]) -> dict[str, dict]:
    ...
    part_fields = {"type", "id", "subtask", "supersedes", "sourceFingerprint", "contractFingerprint", "partSha256"}
    ...
    for record in records:
        if record.get("type") == "subtask_contract":
            ...
            if set(record) != part_fields or not isinstance(part_id, str) or not part_id or not isinstance(subtask_id, str) or not subtask_id or (record.get("supersedes") is not None and (not isinstance(record["supersedes"], str) or not record["supersedes"])) or any(not isinstance(record.get(key), str) or not record[key] for key in ("sourceFingerprint", "contractFingerprint", "partSha256")):
                ...
                record_ids.add(part_id); parts[part_id] = record; contracts[subtask_id] = record
        elif record.get("type") == "subtask_contract_binding":
            binding_id, part_id, checksum = record.get("id"), record.get("partId"), record.get("partSha256")
            if set(record) != binding_fields or not all(isinstance(value, str) and value for value in (binding_id, part_id, checksum)) or binding_id != f"{part_id}.binding":
                ...
                record_ids.add(binding_id); bindings[part_id] = checksum
        else:
            err(2, "invariant 2 (real): source contract record is malformed")
    if set(parts) != set(bindings) or any(bindings[part_id] != part["partSha256"] for part_id, part in parts.items()):
        err(2, "invariant 2 (real): source contract binding is missing or invalid")
    return contracts
def validate(doc: Document, profile: Profile, *, adapters: list[Adapter] | None = None, workspace_root: Path | None = None) -> list[AcddError]:
    ...
    for receipt in doc.receipts:
        ...
        if status in {PASS, INAPPLICABLE} and (prior := _doc.prior_nonterminal(doc.receipts, receipt.get("gate", ""))):
    ...
    contract_receipt = next((receipt for receipt in doc.receipts if receipt.get("gate") == "contract/v1" and receipt.get("status") == PASS), None)
    if contract_receipt:
        bundle = evidence_by_id.get(str(contract_receipt.get("evidence", "")).removeprefix("bundle="))
    ...
    for receipt in doc.receipts:
        ...
        if receipt["status"] == PASS and ({child.get("check") for child in children if child} != check_ids or len(children) != len(check_ids)):
    ...
    for task in doc.subtasks:
        if not task.acceptance.strip() or any(not any(_within(workspace, path, root) for root in input_paths)
    ...
    if contract_receipt:
        for task in doc.subtasks:
            ...
            if task.supersedes and task.supersedes not in subtask_contracts:
                err(6, f"invariant 6 (bounded): replacement {task.id!r} lacks a contracted predecessor")
        for subtask_id in subtask_contracts:
            if subtask_id not in task_ids:
                err(6, f"invariant 6 (bounded): source contract targets missing subtask {subtask_id!r}")
    relationships = {task.id: set(task.depends_on) | ({task.supersedes} if task.supersedes else set())
                     for task in doc.subtasks}
    ...
    for index, left in enumerate(doc.subtasks):
        for right in doc.subtasks[index + 1:]:
            if _conflicts(left, right, relationships):
                err(6, f"invariant 6 (bounded): conflicting subtasks {left.id}/{right.id} need dependency or replacement")
    return errors
