...
def review_scope_ok(dimensions: tuple[str, ...], terminal: dict, records: list[dict]) -> bool:
    scope, performed, reviewed = terminal.get("scope"), terminal.get("performedChecks"), terminal.get("reviewedSessionUuids")
    try:
        raws, sessions = records[:-1], [row["reviewerSessionUuid"] for row in records[:-1]]
        raw_ok = (bool(raws) and terminal.get("reviewerSessionUuid") in sessions and isinstance(reviewed, list)
                  and set(reviewed) == set(sessions) and len(reviewed) == len(set(reviewed)) == len(sessions)
                  and all(set(row) == {"type", "reviewerSessionUuid", "raw"} and row["type"] == "review_raw"
                          and isinstance(row["reviewerSessionUuid"], str) and isinstance(row["raw"], str)
                          and row["reviewerSessionUuid"] != terminal.get("authorSessionUuid") and uuid.UUID(row["reviewerSessionUuid"]) for row in raws))
    except (KeyError, TypeError, ValueError): raw_ok = False
    return (isinstance(scope, list) and scope and all(isinstance(item, str) and item for item in scope)
            ...
            and set(dimensions).issubset(set(performed)) and raw_ok)
def command_outcome_ok(check, terminal: dict) -> bool:
    code = terminal.get("exitCode")
    if not (terminal.get("type") == "command_run" and isinstance(code, int) and not isinstance(code, bool)
