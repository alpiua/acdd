...
class Fingerprint: sha256: str; scope: tuple[str, ...]
def _feed(hasher, tag: str, value: str | bytes) -> None:
    for part in (tag.encode(), value.encode() if isinstance(value, str) else value): hasher.update(len(part).to_bytes(8, "big")); hasher.update(part)
...
def fingerprint_gate(workspace_root: Path, inputs: list[dict], *, types: list[str],
    ...
    for entry in inputs:
        if not isinstance(entry, dict) or not isinstance(entry.get("type"), str) or not isinstance(entry.get("path"), str) or not entry["path"]:
            raise ValueError(f"invalid input entry: {entry!r}")
    selected = sorted((entry["type"], entry["path"]) for entry in inputs if entry.get("type") in types and (files is None or entry.get("path") in files))
...
def _binding_contract(adapter, gate) -> dict:
    bindings = {cid: {**asdict(binding), "promptDigest": adapter.prompt_digest(binding)} for cid, binding in adapter.gates[gate.id].checks.items()} if adapter and gate.id in adapter.gates else {}
    adapter_part = {"id": adapter.id, "role": adapter.role, "artifactDir": adapter.artifact_dir, "checks": bindings} if adapter else None
...
def _subtask_data(task) -> dict:
    return {"id": task.id, "writes": list(task.writes), "reads": list(task.reads),
            "acceptance": task.acceptance, "dependsOn": list(task.depends_on), "supersedes": task.supersedes}
...
def subtask_contract_part(task, evidence_id: str, contract_fingerprint: str) -> dict:
    part = {"type": "subtask_contract", "id": evidence_id, "subtask": task.id, "supersedes": task.supersedes, "sourceFingerprint": subtask_fingerprint(task), "contractFingerprint": contract_fingerprint}
    return {**part, "partSha256": subtask_contract_hash(part)}
def subtask_contract_hash(part: dict) -> str:
    return _digest("acdd/subtask-contract-part/1", {key: value for key, value in part.items() if key != "partSha256"})
