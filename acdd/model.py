...
def load_profile(path: Path) -> Profile:
    ...
    for raw in data.get("gates") or []:
        ...
        reasons = tuple(gate.get("inapplicableReasonCodes") or ())
        if INAPPLICABLE in terminals and not reasons:
            raise AcddError(f"{gate_id} permits inapplicable without reason codes")
        dimensions = gate.get("reviewDimensions") or []
        if not isinstance(dimensions, list) or any(not isinstance(item, str) or not item for item in dimensions):
            raise AcddError(f"{gate_id} reviewDimensions must be a list of non-empty strings")
        ...
        gates.append(Gate(gate_id, owner, checks, tuple(gate.get("invalidatesOn") or ()), terminals, reasons,
                          tuple(dimensions)))
