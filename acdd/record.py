...
def _command_record(*, argv: tuple[str, ...], cwd: Path, evidence_id: str, gate_id: str,
    ...
    try:
        result = subprocess.run(argv, cwd=cwd, text=True, capture_output=True, timeout=timeout_seconds, check=False); exit_code, output = result.returncode, result.stdout + result.stderr
    ...
    record = {"type": "command_run", "evidenceId": evidence_id, "gate": gate_id, "check": check_id, "argv": [_SECRET.sub("<redacted>", arg) for arg in argv], "exitCode": exit_code, "durationMs": int((time.monotonic() - started) * 1000), "output": redacted, "redacted": redacted != output[:MAX_OUTPUT], "timestamp": _doc.utc_now()}
    record.update((key, True) for key, value in (("timeout", timed_out), ("executionError", exec_error)) if value); return record
...
def record_check(*, document: Document, workspace_root: Path, gate: Gate, check_id: str,
    ...
    if check.evidence_kind == "basis":
        basis_scope = [entry["path"] for entry in document.inputs if entry.get("type") in gate.invalidates_on and isinstance(entry.get("path"), str)]
    ...
    artifact_dir = _doc.confined(workspace_root, _doc.relative_ref(workspace_root, adapter.resolve(adapter.artifact_dir)), "artifactDir")
    ...
    terminal = _command_record(argv=argv, cwd=cwd, evidence_id=evidence_id, gate_id=gate.id, check_id=check_id, timeout_seconds=check_binding.timeout_seconds)
    ...
    if not _doc.command_outcome_ok(check, terminal):
        return None, False
    payload = {"kind": check.evidence_kind, "id": evidence_id, "gate": gate.id, "check": check_id, "issuerRole": adapter.role, "artifactSha256": _doc.sha256(artifact_path), "inputFingerprint": fingerprint, "recordedAt": _doc.utc_now(), _KIND_REF[check.evidence_kind]: _doc.relative_ref(workspace_root, artifact_path)}
    if check.evidence_kind == "basis":
...
def record_review(*, document: Document, workspace_root: Path, gate: Gate, check_id: str,
    ...
    if not records or not _doc.terminal_matches(terminal, evidence_id, gate.id, check_id) or terminal.get("type") != "review_terminal" or terminal.get("verdict") != PASS:
        raise AcddError("review transcript terminal must be review_terminal with verdict=pass")
    if str(terminal.get("authorSessionUuid")) != author_uuid or str(terminal.get("reviewerSessionUuid")) != reviewer_uuid:
    ...
    if not _doc.review_scope_ok(gate.review_dimensions, terminal, records):
        raise AcddError(f"review transcript must retain raw reviewer sessions, confirmation of each, a non-empty scope, and performedChecks covering {sorted(gate.review_dimensions)!r}")
    payload = {"kind": "review", "id": evidence_id, "gate": gate.id, "check": check_id, "issuerRole": adapter.role, "transcriptRef": _doc.relative_ref(workspace_root, transcript_path), "artifactSha256": _doc.sha256(transcript_path), "inputFingerprint": fingerprint, "recordedAt": _doc.utc_now(), "authorSessionUuid": author_uuid, "reviewerSessionUuid": reviewer_uuid, "verdict": verdict}
    _doc.append_evidence(document.path, payload)
    return payload
def _artifact_dir(workspace_root: Path, adapter: Adapter) -> Path:
    directory = _doc.confined(workspace_root, _doc.relative_ref(workspace_root, adapter.resolve(adapter.artifact_dir)), "artifactDir")
    directory.mkdir(parents=True, exist_ok=True)
    return directory
def _subtask_contract_records(part: dict) -> tuple[dict, dict]:
    return part, {"type": "subtask_contract_binding", "id": f"{part['id']}.binding", "partId": part["id"], "partSha256": part["partSha256"]}
def _write_subtask_bundle(document: Document, workspace_root: Path, adapter: Adapter, evidence_id: str, contract_fingerprint: str) -> str:
    ...
    parts = [subtask_contract_part(task, f"{evidence_id}.subtask.{index}", contract_fingerprint) for index, task in enumerate(document.subtasks, 1)]
    with artifact.open("x", encoding="utf-8") as handle: handle.writelines(json.dumps(record, sort_keys=True) + "\n" for part in parts for record in _subtask_contract_records(part))
...
def _contract_bundle(document: Document, receipt: dict) -> dict | None:
    ...
    return next((item for item in document.evidence if item.get("id") == evidence_id and item.get("kind") == "bundle"), None)
...
def _safe_source_bundle(workspace_root: Path, bundle: dict | None) -> Path | None:
    if not bundle or not isinstance(bundle.get("subtaskContractBundleRef"), str):
        return None
    try:
        return _doc.confined(workspace_root, bundle["subtaskContractBundleRef"], "subtask contract bundle")
    except AcddError:
        return None

def record_subtask_contract(*, document: Document, profile: Profile, workspace_root: Path, adapter: Adapter, subtask_id: str, evidence_id: str, adapters: list[Adapter] | None = None) -> dict:
    ...
    errors = [error for error in validate(document, profile, adapters=adapters or [adapter], workspace_root=workspace_root) if str(error) != missing]
    if errors:
    ...
    if not _EVIDENCE_ID.fullmatch(evidence_id) or any(item.get("id") == evidence_id for item in records):
    ...
    with artifact.open("a", encoding="utf-8") as handle: handle.writelines(json.dumps(record, sort_keys=True) + "\n" for record in _subtask_contract_records(part))
    return part
def _require_finalizable(document: Document, profile: Profile, adapters: list[Adapter], workspace_root: Path, gate: Gate) -> None:
    errors = validate(document, profile, adapters=adapters, workspace_root=workspace_root)
    stale = f"invariant 4 (state): stale receipt for {gate.id}"; replacing = any(str(error) == stale for error in errors)
...
def finalize_gate(*, document: Document, profile: Profile, adapters: list[Adapter], workspace_root: Path, gate: Gate, evidence_id: str, adapter: Adapter, status: str = "pass", reason_code: str | None = None) -> dict:
    ...
    _require_fresh_id(document, evidence_id)
    fingerprint = fingerprint_for_gate(document, gate, workspace_root, adapter)
    members = [item for item in document.evidence if item.get("gate") == gate.id and item.get("kind") != "bundle" and item.get("inputFingerprint") == fingerprint]
    ...
    if status == INAPPLICABLE:
        ...
        by_check = {item.get("check"): item for item in members if isinstance(item.get("check"), str)}
    ...
    _doc.append_evidence(document.path, payload)
    _doc.upsert_receipt(doc_path=document.path, gate_id=gate.id, status=status, evidence_ref=f"bundle={evidence_id}", fingerprint=fingerprint, recorded_at=_doc.utc_now())
    return payload
