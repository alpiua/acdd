...
|---:|---|---|---|---|
| 10 | command transport | Legacy external callers may still pass a shell command string to the runner. | The Planner wrapper now passes a typed argv vector; retain `--check-command` only for legacy callers and migrate them to repeated `--check-arg`. Keep shell only as an explicitly declared launcher capability. | wrapper fixed; legacy migration remains |
...
| 9 | host binding | Generic ACDD is host-neutral, while the Planner architecture validator currently requires Pi command targets. | Keep the current Planner policy explicit; extract host binding into an adapter capability before supporting Codex or another executor. Do not silently bypass the validator. | documented boundary |
...
| 4 | model routing | Planner defaults already route Terra/high to inspectors and Sol/high to the coordinator. | Measure quality and cost before changing models. Do not move routing into generic ACDD fields or infer it from `runtime`. | preserve |
| 3 | derived refreshes | Derived Planner/Forge/Code Map refreshes can be expensive. | Keep one refresh after candidate freeze and one final handoff refresh; never refresh after each individual edit. | implemented policy |
| 2 | collaboration slots | Inspector concurrency is external to the Codex collaboration tree. | Do not optimize by reducing the four partitions or serializing inspectors. The runner already removes the four-slot bottleneck. | preserve |
